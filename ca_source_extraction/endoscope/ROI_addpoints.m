function [results, center, Cn, PNR, save_avi] = ROI_addpoints(Y, K, options,debug_on, save_avi,coords_in)
...
if(K>0)
    ...
    if strcmpi(seed_method, 'manual') %manually select seed pixels
        ...
        for tmp_k=1:K
            ind_localmax(tmp_k) = sub2ind([d1,d2], coords_in(tmp_k,2), coords_in(tmp_k,1));
        end
